...
export async function placeOrder(
  ...
  try {
    const table = db.select().from(schema.diningTables).where(eq(schema.diningTables.qr_token, qrToken)).get();
    if (!table) return { ok: false, error: "Table not found — scan a valid QR code." };
    const restaurant = db.select().from(schema.restaurants).where(eq(schema.restaurants.id, table.restaurant_id)).get();
    ...
    for (const line of lines) {
      const item = db.select().from(schema.menuItems).where(eq(schema.menuItems.id, line.menuItemId)).get();
      ...
      if (line.variantId) {
        const v = db.select().from(schema.menuItemVariants).where(eq(schema.menuItemVariants.id, line.variantId)).get();
...
export async function cancelItem(qrToken: string, deviceId: string, itemId: number): Promise<ActionResult> {
  ...
  const order = db.select().from(schema.orders).where(eq(schema.orders.id, item.order_id)).get();
  if (!order) return { ok: false, error: "Order not found." };
  if (order.device_id !== deviceId) return { ok: false, error: "You can only cancel your own items." };
  const table = db.select().from(schema.diningTables).where(eq(schema.diningTables.id, order.table_id)).get();
  const restaurant = db.select().from(schema.restaurants).where(eq(schema.restaurants.id, table?.restaurant_id ?? 0)).get();
  ...
  db.update(schema.orderItems).set({ status: "cancelled" }).where(eq(schema.orderItems.id, itemId)).run();
  return { ok: true };
}
...
export async function updateItemStatus(itemId: number, status: string): Promise<ActionResult> {
  ...
  const item = db.select().from(schema.orderItems).where(eq(schema.orderItems.id, itemId)).get();
  if (!item) return { ok: false, error: "Item not found." };
  db.update(schema.orderItems).set({ status }).where(eq(schema.orderItems.id, itemId)).run();
  ...
  if (orderItems.length > 0 && orderItems.every((i) => i.status === "served")) {
    db.update(schema.orders).set({ status: "completed" }).where(eq(schema.orders.id, item.order_id)).run();
  }
  return { ok: true };
}
...
export async function approveOrder(orderId: number): Promise<ActionResult> {
  ...
  if (!order) return { ok: false, error: "Order not found." };
  const session = db.select().from(schema.tableSessions).where(eq(schema.tableSessions.id, order.session_id)).get();
  if (session) {
    db.update(schema.tableSessions).set({ approval_status: "approved" }).where(eq(schema.tableSessions.id, session.id)).run();
  }
  db.update(schema.orders).set({ status: "accepted" }).where(eq(schema.orders.id, orderId)).run();
  return { ok: true };
}
...
export async function generateBill(sessionId: number): Promise<ActionResult> {
  ...
  const restaurant = db.select().from(schema.restaurants).where(eq(schema.restaurants.id, session.restaurant_id)).get();
  const items = db
    .select()
    .from(schema.orderItems)
  ...
  const seq = (db.select({ c: sql<number>`count(*)` }).from(schema.bills).where(sql`${schema.bills.invoice_number} LIKE ${`${dateStr.replace(/-/g, "")}-%`}`).get()?.c ?? 0) + 1;
  ...
  db.update(schema.tableSessions).set({ status: "billed", closed_at: new Date().toISOString() }).where(eq(schema.tableSessions.id, session.id)).run();
  db.update(schema.orders).set({ status: "completed" }).where(eq(schema.orders.session_id, session.id)).run();
...
export async function toggleItemAvailability(itemId: number): Promise<ActionResult> {
  const item = db.select().from(schema.menuItems).where(eq(schema.menuItems.id, itemId)).get();
  if (!item) return { ok: false, error: "Item not found." };
  db.update(schema.menuItems).set({ is_available: item.is_available ? 0 : 1 }).where(eq(schema.menuItems.id, itemId)).run();
  return { ok: true };
}
...
export async function pollSession(qrToken: string, deviceId: string): Promise<SessionPayload> {
  ...
  const orders = db.select().from(schema.orders).where(eq(schema.orders.session_id, session.id)).orderBy(asc(schema.orders.order_number)).all();
  const items = db.select().from(schema.orderItems).where(eq(schema.orderItems.session_id, session.id)).orderBy(asc(schema.orderItems.id)).all();
  return {
...
export async function getBillingData() {
  ...
  const open = getBillableSessions().map((b) => ({ ...b }));
  const closed = getClosedBills().map((b) => ({
    id: b.id,
...
export async function getMenuAdmin() {
  ...
  const categories = db.select().from(schema.menuCategories).orderBy(asc(schema.menuCategories.sort_order)).all();
  const items = db.select().from(schema.menuItems).orderBy(asc(schema.menuItems.sort_order)).all();
...
export async function getTablesList() {
  const tables = db.select().from(schema.diningTables).orderBy(asc(schema.diningTables.table_number)).all();
  const restaurant = db.select().from(schema.restaurants).limit(1).get();
  return { tables, restaurant };
}

export async function getSettings() {
  const r = db.select().from(schema.restaurants).limit(1).get();
  return r
    ? {
        gst_rate: r.gst_rate,
...
export async function regenerateQrToken(tableId: number): Promise<ActionResult> {
  ...
  db.update(schema.diningTables).set({ qr_token: token }).where(eq(schema.diningTables.id, tableId)).run();
  return { ok: true, data: { token } };
}

export async function getTotalOrders() {
  const sessions = db.select().from(schema.tableSessions).where(eq(schema.tableSessions.status, "billed")).orderBy(desc(schema.tableSessions.closed_at)).all();
  ...
  for (const s of sessions) {
    ...
    const orders = db.select().from(schema.orders).where(eq(schema.orders.session_id, s.id)).all();
    const items = db.select().from(schema.orderItems).where(eq(schema.orderItems.session_id, s.id)).all();
